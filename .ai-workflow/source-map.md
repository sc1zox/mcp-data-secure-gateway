...
| --- | --- | --- | --- |
| `src/index.ts` | 254 | Start, Boot-Reihenfolge, registriert Geheimnisse beim `EgressGuard` | local-only |
| `src/config.ts` | 493 | Konfiguration laden, `${VAR}`-Ersetzung gegen die Umgebung | local-only |
| `src/core/orchestrator.ts` | 500 | Koordiniert Kollaborateure; öffentliche Signatur für Einstiegspunkt, Freigabe-Server und MCP-Oberfläche | boundary |
| `src/core/types.ts` | 470 | `Internal*`/`Public*`-Typen, `ActionStatus`, `REDACTION_PLACEHOLDERS` | boundary |
| `src/core/egress.ts` | 517 | Aufbau jeder Antwort an Hermes nach Whitelist; `EGRESS_NOTES`-Katalog; `EgressGuard` | boundary |
...
| `src/core/actionExecutor.ts` | 425 | Einziger Besitzer von `staged`; führt eine freigegebene Aktion aus (Inv. 7 + 12) | boundary |
| `src/core/actionPreparation.ts` | 404 | Bereitet Send- und Zusammenfassungsaktionen bis zur lokalen Freigabe vor | boundary |
| `src/core/preparationLimits.ts` | 171 | Schutz gegen Freigabeflut: Rate Limit, Obergrenze offener Aktionen, exakte Duplikate | hermes-facing |
| `src/core/selectionFlow.ts` | 403 | Öffnet, parkt, löst und storniert lokale Auswahlen (Inv. 9) | boundary |
| `src/core/localViews.ts` | 313 | Lokale Projektion einer Aktion/Auswahl für die Freigabeoberfläche; erreicht Hermes nie | local-only |
| `src/mcp/hermesServer.ts` | 555 | Die 7 MCP-Werkzeuge, mit denen Hermes das Gateway anspricht | hermes-facing |
| `src/judge/judge.ts` | 685 | Lokale semantische Bewertung, liefert nur schemagebundenes JSON | local-only |
| `src/judge/ollamaClient.ts` | 556 | Ollama-Client; kein Cloud-Fallback bei Ausfall (Inv. 10) | local-only |
| `src/judge/prompts.ts` | 459 | Prompt-Vorlagen für den Judge (Inv. 11: Inhalt ist Daten, keine Anweisung; fester Rahmenmarker, keine Nonce) | local-only |
...
| `src/attachments/pipeline.ts` | 367 | Stufenreihenfolge, Budgetarithmetik, Kandidatenannahme; fail-closed, ohne Kaskadierung | local-only |
...
| `src/store/actionStore.ts` | 314 | Aktionsstatus, erlaubte Übergänge, `ActionImmutabilityError` (Inv. 12) | local-only |
